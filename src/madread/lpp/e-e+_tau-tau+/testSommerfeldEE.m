...
E = csvread(sprintf('%sbeam_energy_global.csv', data_dir), 1, 0);
% Infer velocity.
v = sqrt(1 - mtau^2./E.^2);

% Read in cross section data, in units of pb.
cs_data = csvread(sprintf('%scross_section_global.csv', data_dir), 1, 0);
cs = cs_data(:,1);
dcs = cs_data(:,2);
...
title('cross sections for $e^-e^+\rightarrow\tau^-\tau^+$ production')
...
title('Sommerfeld enhancement for $e^-e^+\rightarrow\tau^-\tau^+$ production')